...
gamma=0.3
eta=0.3
t_0=10000#100#10 #1000
n=1000000  #50000000 #1000000
B=0.01
alpha = 0.01
...
def basic_term(x):
    J_i =  -1.0/ (-B * a * np.power(x[1],a-1))
    J_j =  -1.0/ (-B * a * np.power(x[2],a-1))
    Q_i = J_i * b * np.power(x[1], b-1) * alpha
    Q_j = J_j * b * np.power(x[2], b-1) * alpha
...
def plotting(weights,final_times):
    ...
    plt.tight_layout()
    #plt.savefig('P_12_6.pdf',dpi=300)
    plt.show()
...
for weight in weights:
    ...
    times_simulation_edge,x_edge=simulation(A_edge)
    ...
    L_im = (weight) * Q_i * Q_j 
    J_i_list.append(J_i)
    Eim_list.append(L_im)

np.save('edge_J_i_list_'+str(int(100*a))+'_'+str(int(100*b)), J_i_list)
np.save('edge_Eim_list_'+str(int(100*a))+'_'+str(int(100*b)), Eim_list)

final_times_R=np.mat(final_times_R)
save_dict(final_times_R, 'edge_P_'+str(int(100*a))+'_'+str(int(100*b)))
save_dict(weights, 'edge_P_weight_'+str(int(100*a))+'_'+str(int(100*b)))
