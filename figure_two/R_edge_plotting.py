...
final_times = load_dict('edge_R_'+str(int(100*a))+'_'+str(int(100*b)))
weights = load_dict('edge_R_weight_'+str(int(100*a))+'_'+str(int(100*b)))
...
ax.scatter(weights,times,s=250,marker = 's',c=colors[0],label = 'Simulation',alpha =1.0)
ax.loglog(weights,times,c=colors[0],linewidth=4,linestyle='-',alpha =1.0)
...
times=np.array((final_times[:,2]).T.tolist()[0])*30
...
#ax.legend([r'$Simulation$',r'$Theory$',r'$d_i^{\theta}$'],fontsize=15)
plt.tight_layout()
plt.savefig('edge_R_'+str(int(100*a))+'_'+str(int(100*b))+'.pdf',dpi=300)
#plt.savefig('label_Test_two.pdf',dpi=300)
plt.show()