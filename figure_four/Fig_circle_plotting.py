...
def simulation(A,active_node,fun):
    def F(A,x):
        if(fun=='R'):
            #return np.mat(-1*np.power(x,1)+A*(np.power(x,1)/(1+np.power(x,1))))
...
if __name__=='__main__':
    ...
    total_plotting(model+'2',xs_1,active_node,positions_1,locations_1,edges_1,D_1,D_numbers)
    total_plotting(model,xs_2,active_node,positions_2,locations_2,edges_2,D_2,D_numbers)
    total_plotting(model+'1',xs_3,active_node,positions_3,locations_3,edges_3,D_3,D_numbers)  
